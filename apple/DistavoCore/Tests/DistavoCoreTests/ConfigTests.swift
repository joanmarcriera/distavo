...
final class ConfigTests: XCTestCase {
    ...
    func testDefaultsMatchPython() {
        ...
        XCTAssertEqual(c.summarise.promptStyle, .classic)
    ...
    func testPromptStyleDecodesAndExistingModelKept() throws {
        ...
        XCTAssertEqual(c.summarise.promptStyle, .classic, "existing configs keep the classic prompt")
        XCTAssertEqual(Config.recommendedForThisMac(embeddedSupported: true, memoryBytes: 1 << 34).summarise.promptStyle, .factsFirst)

        try #"{"summarise": {"prompt_style": "facts_first"}}"#.write(to: url, atomically: true, encoding: .utf8)
