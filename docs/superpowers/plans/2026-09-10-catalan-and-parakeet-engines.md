...
- [ ] **Step 5: Bake-off** — **finding from Task 14 (2026-09-10):** today's transcript of the 2026-07-23 meeting (`Meeting_2026-07-23_10.58.50.transcript.clean.txt`, 7,144 words) is 98 % English because the config's `language = "en"` made Whisper *translate* a Catalan/Spanish meeting; it is not a baseline. Compare like with like using the live test (Task 14's env vars), on the 2026-07-23 recording: (a) `DISTAVO_PIPELINE_MODEL=large-v3-turbo DISTAVO_PIPELINE_LANGUAGE=auto` (Whisper's own detection, no translation), (b) `bsc-los` with `ca`, (c) `bsc-los` with `auto`, and on the 2026-09-09 English call: (d) `parakeet-tdt-v3` with `en` and (e) `large-v3-turbo` with `en`. Record per run: wall-clock, model load, word count, language mix of the output (the chunk tagger in the session scratch is fine: en/ca/es/mixed percentages), and summary quality of the resulting note. Scoring: for the English call (d, e) run `~/Documents/Distavo/work/bakeoff/score.py`'s checks (it is written for exactly that recording: copy the two notes into a scratch `out/` folder and run it there); for the Catalan meeting (a, b, c) fill a structured checklist per note — every section present and non-empty (count), number of action items, number of named people/organisations, whether the note is in the meeting's language, whether the transcription-corrections section is non-empty, obvious artefacts (untranslated fragments, repeated lines) — and record only those counts. You may read the notes to score them; never quote or paraphrase their content in the report or the results file, and never open the transcripts. Rule 4 of the router (ca+en → LoS) stays only if (b) or (c) beats (a) on that checklist; the final go/no-go on the Catalan note is Marc's read, so the results file must name the three note files for him. Ollama at `http://127.0.0.1:11434` with `gemma4:26b` is running on this Mac for the summaries. A language-mix tagger for the transcripts is at `.superpowers/sdd/2026-09-10-catalan-and-parakeet-engines/language-mix.py` (counts only). Write `docs/superpowers/plans/2026-09-10-bakeoff-results.md` with the numbers (no transcript text). Go/no-go per spec §8.
